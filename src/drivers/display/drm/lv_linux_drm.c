...
bo_create(drm_device_t *dev, int width, int height, int format)
{
    ...
    LV_LOG_INFO("Created bo: %d, %dx%d", bo->fb_id, width, height);
...
#define DRM_CONNECTOR_CFG_PATH_DEFAULT  "/tmp/drm_connector.cfg"
    ...
    if ((access(path, F_OK) == 0))
    {
        ...
        if (read(fd, buf, sizeof(buf)) <= 0)
            LV_LOG_INFO("read failed, use default connector");
        close(fd);
    }
...
#define DRM_MODE_CFG_PATH_DEFAULT   "/tmp/drm_mode.cfg"
    ...
    if ((access(path, F_OK) == 0))
    {
        fd = open(path, O_RDONLY);
        if (read(fd, buf, sizeof(buf)) <= 0)
            LV_LOG_INFO("read failed, use default mode");
        close(fd);
    }
...
drm_get_connector(drm_device_t *dev, int connector_id)
{
    ...
    LV_LOG_INFO("Connector id: %d, %sconnected, modes: %d", connector_id,
...
drm_find_best_crtc(drm_device_t *dev, drmModeConnectorPtr conn)
{
    ...
    if (encoder)
    {
        ...
        LV_LOG_INFO("Preferred crtc: %d", preferred_crtc_id);
    ...
    {
        encoder = drmModeGetEncoder(dev->fd, conn->encoders[i]);
        if (encoder)
            crtcs_for_connector |= encoder->possible_crtcs;
        drmModeFreeEncoder(encoder);
    }
    LV_LOG_INFO("Possible crtcs: %x", crtcs_for_connector);
...
static drmModePlanePtr get_preferred_plane(drm_device_t *dev, int plane_id, int pipe)
{
    ...
    LV_LOG_INFO("Check plane: %d, possible_crtcs: %x", plane_id,
...
drm_find_best_mode(drm_device_t *dev, drmModeConnectorPtr conn)
{
    ...
    LV_LOG_INFO("Preferred mode: %dx%d", preferred_width, preferred_height);

    for (i = 0; i < conn->count_modes; i++)
    {
        LV_LOG_INFO("Check mode: %dx%d",
...
static void drm_flush(lv_display_t *disp, const lv_area_t *area, uint8_t *color_p)
{
    ...
    if (!dev->disp_rot)
    {
        ...
        for (y = area->y1; y <= area->y2; y++)
        {
            lv_color_t *ptr = (lv_color_t *)(dev->flush_buf + (y * dev->flush_buf_stride + area->x1) *
                                             (LV_COLOR_DEPTH >> 3));
